...
- The `0.y.z` scheme is the current pre-stability phase. A real `1.0.0` is on the table once the feature set and APIs settle.

## Releasing

`./scripts/bump-and-push.sh <patch|minor|major|x.y.z>` bumps the version, tags
it, and pushes the branch + tag. The pushed `v*` tag triggers the Release
workflow, which builds both zips and creates the GitHub release with them
attached.

To build and release from your machine instead of via CI (or to re-upload assets
to an existing release), run `scripts/release.sh` directly — it builds both
targets and creates/updates the GitHub release.
...
[Starlight](https://starlight.astro.build) and lives in `site/`.
