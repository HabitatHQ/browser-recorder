...
the same self-contained `.zip` report as the extension (a slightly slimmer
subset). Two modes:
...
Both modes produce the same `.zip` shape, which can be opened in any browser.

## Install

The package is not published to npm yet. From a clone of the repo:
...
**Not captured by the CLI** (the extension captures these; the CLI does not):
...
The CLI output is intentionally lighter than the extension's (no `report.html`
viewer, no merged `events.json`, no video). For the full artifact set, use the
extension.
