...
export function appendEventWithRetentionPolicy(
  ...
  while (events.length > MAX_EVENT_COUNT) {
    const dropIndex = findOldestEventIndexByPriority(events, ["console", "action", "network"]);
...
function getNavigationUrl(event: Extract<DebuggerEvent, { kind: "action" }>): string | null {
...
function enforceKindCap(events: DebuggerEvent[], kind: DebuggerEvent["kind"]): void {
