import { MAX_NETWORK_BODY_LENGTH, MAX_TEXT_LENGTH, MAX_URL_LENGTH } from "./constants";
...
export function normalizeStoredSession(value: unknown): StoredDebuggerSession | null {
  ...
  const recordingStartedAt =
    value.recordingStartedAt === null ? null : asOptionalNumber(value.recordingStartedAt);
...
export function normalizeStoredReplayBuffer(value: unknown): StoredReplayBuffer | null {
  if (!isRecord(value)) return null;

  const tabId = asOptionalNumber(value.tabId);
  const lastTouchedAt = asOptionalNumber(value.lastTouchedAt);
  if (tabId === undefined || tabId < 0 || lastTouchedAt === undefined || lastTouchedAt < 0) {
    return null;
  }
