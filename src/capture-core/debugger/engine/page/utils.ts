...
export const truncate = (value: string, maxLength = MAX_TEXT_LENGTH): string => {
...
export const getElementTarget = (
  ...
  while (el && el.tagName.toLowerCase() !== "html") {
    const seg = elementSelector(el);
    parts.unshift(seg);
    // Stop once we've anchored to an ID — no need to go further up
    if (el.id) break;
...
export const toAbsoluteUrl = (value: string, reporter: Reporter): string | null => {
  try {
